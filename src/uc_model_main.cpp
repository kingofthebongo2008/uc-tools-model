...
#include <uc/import/fbx/indexed_mesh.h>
...
#include <uc/import/geo/merge_attributes_indexed_mesh.h>
#include <uc/import/fbx/multi_material_mesh.h>

#include <uc/import/geo/skinned_mesh.h>
#include <uc/import/geo/merge_attributes_skinned_mesh.h>
    ...
    namespace import
    {
        namespace model
        {
            ...
            static uc::lip::texture2d create_texture_2d(const file_name_t& input_file_name, const std::string& texture_format)
            {
                ...
                auto view = texture::string_to_view_format(texture_format);

                if (storage == lip::storage_format::unknown)
                {
                    return texture::create_texture_2d(input_file_name);
                }
                else
                {
                    return texture::create_texture_2d(input_file_name, storage, view);
                }
            }
            ...
            static uc::lip::texture2d_mip_chain create_texture_2d_mip_chain(const file_name_t& input_file_name, const std::string& texture_format)
            {
                ...
                if (storage == lip::storage_format::unknown)
                {
                    return texture::create_texture_2d_mip_chain(input_file_name);
                }
                else
                {
                    return texture::create_texture_2d_mip_chain(input_file_name, storage, view);
                }
            }
            ...
            static std::vector<std::string> materials(const std::vector<std::string>& names)
            {
                ...
                {
                        return gx::import::fbx::material_name(i);
                });
            ...
            template <typename mesh_create_functor, typename model_create_functor, typename copy_attributes> void convert_multi_textured_mesh(const file_name_t& input_file_name, const file_name_t& output_file_name, const mesh_create_functor& create_mesh, const model_create_functor& create_model, const copy_attributes& _copy_attributes, const std::vector<std::string>& texture_file_name, const std::vector<std::string>& texture_format)
            {
                auto m = create_model();
                auto mesh = create_mesh(input_file_name);
                ...
                g.run([&m, &mesh, &input_file_name, &mats, &create_mesh, &_copy_attributes]()
                    {
                        _copy_attributes(m.get(), mesh.get(), mats);
                    });
            ...
            /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            static void copy_attributes_normal_skinned_model(lip::normal_skinned_model* d, const gx::import::geo::skinned_mesh* s, const std::vector<std::string>& mats)
            {
            ...
            static void convert_normal_skinned_model_fbx(const std::string& input_file_name, const std::string& output_file_name, const std::vector< std::string>& texture_file_name, const std::vector< std::string>& texture_format)
            {
                ...
                {
                    copy_attributes_normal_skinned_model(d, s, mats);
                };
...
int32_t main(int32_t argc, const char* argv[])
{
    ...
    try
    {
        ...
        auto&& om = model::build_option_map(argc, argv);
        ...
        auto input_model = model::get_input_model(vm);
        auto output_model = model::get_output_model(vm);

        auto model_type = model::get_model_type(vm);
        ...
        auto convertor = model::get_convertor(model_type);
        ...
        
        {
            ...
            {
                tie(textures, texture_formats) = model::clean_duplicate_textures(model::get_textures(vm), model::get_texture_formats(vm));
            }
